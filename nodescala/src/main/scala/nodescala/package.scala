  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      f.value match {
        ...
        case Some(t) => t match {
          case Success(x) => x
          case failure@Failure(_) => throw new NoSuchElementException
        }
      }
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete{
        case Success(r) => p complete Try(cont(f))
        case Failure(r) => p.failure(r)
      }
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete{
        case Success(r) => p complete Try (cont(Success(r)))
        case Failure(r) => p failure r
      }
      p.future
    }
